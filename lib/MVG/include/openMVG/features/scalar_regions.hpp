...
// 存储 特征点 和 标量描述子 。
// Scalar_Regions继承自Regions，并自定义为模板类，其中模板参数主要有三个：
// 特征点类型 、 描述子类型 、 描述子维度
...
public:
  ...
  // FeatT的类型一共有三种：
  //  1. PointFeature, 只包含坐标(x, y)；
  //  2. SIOPointFeature, 包含坐标(x, y)，还有尺度和方向(scale, orientation)；（常用）
  //  3. AffinePointFeature, 包含坐标(x, y)，还有a, b, c三个参数，用于计算两条直线和角度。
  /// Region type
  using FeatureT = FeatT;
  /// Region descriptor  描述子类型 、维度 。
