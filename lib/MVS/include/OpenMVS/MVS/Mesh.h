...
public:
	...
	VertexFacesArr vertexFaces; // 对每个顶点，与包含其的所有 for each vertex, the list of faces containing it (optional)
	...
	TexCoordArr faceTexcoords; // 每个face的纹理坐标  for each face, the texture-coordinates corresponding to the contained vertices (optional)
...
struct TRasterMesh : TRasterMeshBase<DERIVED> {
	...
	void Project(const Mesh::Face& facet) {
		// project face vertices to image plane     投影face的顶点到图像平面上
		for (int v=0; v<3; ++v) {
			// skip face if not completely inside   如果face没有完全包含在图像内则跳过
			if (!static_cast<DERIVED*>(this)->ProjectVertex(vertices[facet[v]], v))
				return;
		}
		// draw triangle    将三角面内的像素栅格化插值
