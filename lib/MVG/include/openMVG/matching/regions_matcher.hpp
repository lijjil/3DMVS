...
// 调用匹配方法的接口类 。包含两个匹配方法，一个是最近邻，另一个是最近邻比例法。
class RegionsMatcher
{
   ...
   */
  virtual bool MatchDistanceRatio       // 最近邻比例法
  (
...
// openMVG还封装了一个工厂类，这个类主要就是调用 RegionsMatcherT 来创建各种对应的匹配方法
/**
...
// 定义了一个模板类，类型为可变的匹配方法，包括bruteForce，ANN-L2等。
/**
