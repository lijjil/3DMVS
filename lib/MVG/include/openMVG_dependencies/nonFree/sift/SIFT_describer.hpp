...
inline void siftDescToUChar(
  ...
  Descriptor<unsigned char,128> & descriptor,       // 描述子128维
...
public:
  ...
  struct Params
  {
    Params(
      int first_octave = 0,
      int num_octaves = 6,      //影像金字塔的层数
      int num_scales = 3,       //影像金字塔的每层的尺度（通过高斯滤波形成不同的尺度）
      float edge_threshold = 10.0f,     // 过滤掉边缘效应的阈值。edgeThreshold越大，特征点越多（被多滤掉的越少）。
      float peak_threshold = 0.04f,     // 最小值的比值
      bool root_sift = true
    ):
